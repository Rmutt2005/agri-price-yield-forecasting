...
import { normalizePriceRecords } from "@/lib/application/ingestionService";
...
export async function POST(request: Request) {
  ...
  if (!hasPermission(user.role, "ingestion:manage")) {
    return NextResponse.json(
      { error: "FORBIDDEN", message: "ไม่มีสิทธิ์นำเข้าข้อมูลราคา" },
      { status: 403 },
    );
  }
  ...
  const body =
    payload && typeof payload === "object" && !Array.isArray(payload)
      ...
      : {};
  const normalized = await normalizePriceRecords(body.sourceKey, body.records);
  const accepted = normalized.records.length > 0
    ? await observationRepository.savePrices(normalized.records)
    : 0;
