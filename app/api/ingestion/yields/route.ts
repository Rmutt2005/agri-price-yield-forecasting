...
import { normalizeYieldRecords } from "@/lib/application/ingestionService";
...
export async function POST(request: Request) {
  ...
  if (!hasPermission(user.role, "ingestion:manage")) {
    return NextResponse.json(
      { error: "FORBIDDEN", message: "ไม่มีสิทธิ์นำเข้าข้อมูลผลผลิต" },
      { status: 403 },
    );
  }
  ...
  const body =
    payload && typeof payload === "object" && !Array.isArray(payload)
      ...
      : {};
  const normalized = await normalizeYieldRecords(body.sourceKey, body.records);
  const accepted = normalized.records.length > 0
    ? await observationRepository.saveYields(normalized.records)
    : 0;
